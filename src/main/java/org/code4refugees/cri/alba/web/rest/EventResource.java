...
/**
 * REST controller for managing {@link org.code4refugees.cri.alba.domain.Event}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Event> partialUpdateEvent(@PathVariable(value = "id", required = false) final Long id, @RequestBody Event event)
        ...
        Optional<Event> result = eventRepository
            ...
            .map(existingEvent -> {
                ...
                if (event.getUrgencyOfMedicalTreatment() != null) {
                    existingEvent.setUrgencyOfMedicalTreatment(event.getUrgencyOfMedicalTreatment());
                }
                if (event.getNeedForLegalAssistance() != null) {
                    existingEvent.setNeedForLegalAssistance(event.getNeedForLegalAssistance());
                }
